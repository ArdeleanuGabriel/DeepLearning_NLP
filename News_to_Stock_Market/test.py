...
file_path = 'df_0_news_with_keywords.parquet'
output_file = 'df_0_news_with_keywords_and_sentiment.parquet'
...
def process_chunk(chunk_file, output_chunk_file):
    ...
    df_chunk['text_sentiment'] = df_chunk['text'].apply(extract_sentiment)
...
def merge_chunks(chunk_files, output_file):
    dfs = [pd.read_parquet(chunk_file) for chunk_file in chunk_files if '_sentiment_processed' in chunk_file]
...
if __name__ == '__main__':
    ...
    for chunk_file in chunk_files:
        output_chunk_file = chunk_file.replace('.parquet', '_sentiment_processed.parquet')
    ...
    print("Keywords and sentiment added. File saved to:", output_file)